"""Tool execution + session side effects for the ReAct loop (FR-5.1, FR-7, FR-2).

`agent.py` calls `execute_tool` for every Claude `tool_use` block. Business
errors from app.tools.registry (`order_not_found`, `not_eligible`, …) are
successful results; only unknown tools and raised exceptions count toward
the consecutive-error auto-escalate threshold (FR-5.1).

Tiered disclosure (FR-2) is enforced here via `gate_tool_access` before any
Tier-1 or cross-customer call can reach the tool body.
"""
...
from app.session.disclosure import gate_tool_access, tier0_order_view
...
def _prepare_input(
    ...
    """Copy tool input; for escalate, only verified customer_id may pass."""
    prepared = dict(tool_input)
    if name == "escalate_to_human":
        prepared["customer_id"] = state.verified_customer_id
    return prepared
...
def _apply_side_effects(
    ...
    if name == "lookup_order" and "error" not in result:
        ...
        # Tier-0: strip customer_id until this session owns the order (FR-2.1/2.5).
        owner = result.get("customer_id")
        if not (state.is_verified() and owner == state.verified_customer_id):
            result = tier0_order_view(result)

    if name == "verify_customer" and result.get("verified"):
        customer_id = result.get("customer_id")
        if customer_id:
            state.verify(customer_id)
...
def execute_tool(
    ...
    state.record_tool_success()
    return _apply_side_effects(name, prepared, result, state)
